#!/usr/bin/env python3
"""
Optuna hyperparameter optimization CLI for E(3)-VERDE.

Calls ``train_model()`` from ``e3verde.training`` with Optuna TPE + MedianPruner.

Usage:
    python -m e3verde.hpo config.json                   # Run optimization
    python -m e3verde.hpo config.json --n_trials 100    # Custom trial count
    python -m e3verde.hpo config.json --resume          # Resume from checkpoint

The config JSON provides dataset settings (``data`` section) and baseline values.
Model and train hyperparameters are overridden during search.
...
def suggest_hyperparameters(trial, base_cfg):
    ...
    loss_function = trial.suggest_categorical("loss_function", ["l1"])  # L1 only in paper HPO runs
    ...
    train_cfg = TrainConfig(
        ...
        patience=base_cfg.train.patience,
...
def main():
    ...
    # Save study after EVERY trial (protects against SLURM SIGKILL).
    # SIGKILL does NOT trigger finally: blocks, so per-trial saving is required.
    ...
    if completed:
        ...
        if not args.no_retrain:
            ...
            logging.info(f"Run production: python -m e3verde.run {best_config_path}")
    else:
